...
def api_nav(token, ship, wp):
    ...
    try:
        return STClient(token).post(
            f"/my/ships/{ship}/navigate", body={"waypointSymbol": waypoint}
        )
    except RuntimeError as err:
        message = str(err)
        if any(hint in message for hint in ("'course'", '"course"', "course.destination")):
            return STClient(token).post(
                f"/my/ships/{ship}/navigate", body={"course": {"destination": waypoint}}
            )
        raise
        ...
        if not fleet:
            ...
            for ship in filtered:
                ...
                with st.expander(f"🚢 {sym} — {ship.get('registration',{}).get('role','?')}"):
                    ...
                    dest_input = cc4.text_input("Waypoint", value=nav.get("waypointSymbol",""), key=f"wp_{sym}")
                    if cc4.button("Navigate", key=f"n_{sym}"):
                        dest_normalized = normalize_symbol(dest_input)
                        if not dest_normalized:
                            toast_warn("Enter a waypoint symbol before navigating.")
                        else:
                            try:
